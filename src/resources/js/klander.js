klander = function()
{
	...
	this.ajaxSubmit = function(params)
	{
		...
		if(this.pageType === 'checkoutPage')
		{
			...
			else if(paySource === 'PAYPAL')
			{
				...
				params.couponCode = this.getValue('couponCode');
			}
	...
	*/
	...
	{
		var nodes = document.getElementsByClassName('kformPresellLink');
		for(var i = 0;i<nodes.length;i++)
		{
	...
	this.loadInsureShip = function()
	{
		var cart = this.cart;
		try{
			this.getInput('insureShip').addEventListener('click',function(){cart.displayTotals();});
		}catch(e){}
	};
	...
	{
		var params = this.validator.getValues();
