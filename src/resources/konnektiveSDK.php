...
class KonnektiveSDK
{
	...
	{	
		...
		$disableBack = $requirePayInfo = $autoImportLead = $isShoppingCart = 'false';
...
SignInButton = OffAmazonPayments.Button("AmazonPayButton", "<?php echo $amazonSellerId; ?>", {
	...
	function echoKountPixel()
	{
		$sessionId = $this->getSessValue('sessionId');
		$sessionId = substr($sessionId,0,32);
		echo "<iframe width=1 height=1 frameborder=0 scrolling=no src='https://lp2.konnektive.com/logos/logo.htm?m=[KOUNT_MERCHANTID]&s={$sessionId}&c=[companyName]'>
<img width=1 height=1 src='https://lp2.konnektive.com/logos/logo.gif?m=[KOUNT_MERCHANTID]&s={$sessionId}&c=[companyName]'>
</iframe>";
	}
	...
	function importClick($pageType)
	{
		...
		if(is_object($response))
		{
			if($response->result == 'SUCCESS')
			{
				...
				if($pageType == 'checkoutPage' && !empty($this->config->kountMerchantId) && !empty($this->config->enableFraudPlugin) && !$this->getSessValue('kountPixelFired'))
				{
					if(empty($ret->pixel))
						$ret->pixel = '';
	...
	function confirmPaypalOrder($token,$payerId)
	{
		...
		$params->couponCode = $this->getSessValue('paypalCouponCode');
		...
		{
			$this->setSessValue('order',$response->message);
			$loc = $this->getPageUrl('thankyouPage');
			$this->redirect($loc);
		}		
	}
...
<?php } 
	...
	{
		if($order = $this->getOrder())
			return number_format($order->salesTax,2);	
	}
